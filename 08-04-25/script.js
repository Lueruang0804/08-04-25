...
(function () {
  ...
  (function initBackground() {
    ...
    // ---------- Shooting stars: rare diagonal streaks across the sky ----------
    function spawnShootingStar() {
      var w = window.innerWidth, h = window.innerHeight;
      var goingRight = Math.random() > 0.5;
      var angle = (Math.PI / 6) + Math.random() * (Math.PI / 6); // 30–60° from horizontal
      var speed = 9 + Math.random() * 6;
      var startX = goingRight ? Math.random() * w * 0.4 : w - Math.random() * w * 0.4;
      shootingStars.push({
        x: startX,
        y: Math.random() * h * 0.3,
    ...
    function draw() {
      ...
      // Shooting stars: fading streak + bright head, gone in under a second
      for (var k = shootingStars.length - 1; k >= 0; k--) {
        var sh = shootingStars[k];
        sh.x += sh.vx;
        sh.y += sh.vy;
        sh.life -= 0.018;
        if (sh.life <= 0 || sh.x < -sh.length || sh.x > w + sh.length || sh.y > h + sh.length) {
          shootingStars.splice(k, 1);
          continue;
        }
        var speedMag = Math.sqrt(sh.vx * sh.vx + sh.vy * sh.vy);
        var ux = sh.vx / speedMag, uy = sh.vy / speedMag;
        var tailX = sh.x - ux * sh.length;
        var tailY = sh.y - uy * sh.length;
        var alpha = Math.max(sh.life, 0);
        var trailGrad = ctx.createLinearGradient(tailX, tailY, sh.x, sh.y);
        trailGrad.addColorStop(0, 'rgba(248, 248, 248, 0)');
        trailGrad.addColorStop(1, 'rgba(248, 248, 248, ' + alpha.toFixed(2) + ')');
        ctx.beginPath();
